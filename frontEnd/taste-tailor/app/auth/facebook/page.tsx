...
function FacebookCallbackPage() {
  ...
  useEffect(() => {
    // This page is loaded by the browser after Facebook redirects back to the frontend.
    // Now, we make a fetch request to our backend to get the user data
    // using the session/cookie that the backend set during the /facebook/auth/ call.
    ...
    const fetchUserData = async () => {
      try {
        ...
        const response = await fetch('http://localhost:5000/taste_tailor_google_api', { // Note: Still fetching from taste_tailor_google_api as it's generic session check endpoint
          credentials: 'include'
        });
